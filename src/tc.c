...
volatile uint32_t A[32] __attribute__ ((aligned (64))) __attribute__ ((section (".data"))) = {1654615998,1806341205,173879092,1112038970,2087043557,1739178872,1302718217,2046968324,1537810351,938204377,598176026,1210484339,600203567,407295012,1075916535,631194409,1332073689,424185324,316721330,1418186270,2027837527,432508404,1519522183,1864753826,1358054485,878225224,2048741382,1901353491,1118805955,267488770,60308648,400599612};
volatile uint32_t W[32] __attribute__ ((aligned (64))) __attribute__ ((section (".data"))) = {1712934065,4908357,2119900799,1430804514,952210990,1024847607,612008755,1924014660,468399889,1294704107,1250224899,536057929,393556845,1653137829,1361705852,1039842312,1047589226,1396742090,270515404,820626892,391769539,345512145,1374624034,2101470722,1429152570,872745308,1235478542,1911213317,1246955724,789710164,813429542,801997237};
volatile uint32_t result[64] __attribute__ ((aligned (64))) __attribute__ ((section (".data"))) = {0};
volatile uint32_t expected[64] __attribute__ ((aligned (64))) __attribute__ ((section (".data"))) = {325473286,1406991641,11274198,1154471106,2116909427,1050136684,1843866275,696580535,1434839804,356467683,1179629868,36465208,209192502,1287827241,531781094,1540458226,2004204986,502204918,2030580771,1387259291,440307919,1889805443,339941718,1071958254,831914816,1527559948,1072401113,340548449,1751904138,49372102,1712743955,1303492523,1108115834,45492325,1371984554,363135422,115931237,1179799930,354593717,1363245217,1034237931,2099224308,779052091,1779218223,190863798,1272639657,413796966,1556732274,639592646,1283841162,677568863,63254759,1660621273,140617109,1820760128,1193363960,1505156695,858548763,1472944078,2087487094,1869852682,99157062,1628322195,1252649003};
            ...
            for (int i = 0; i < I_PRIME; i++) {
                for (int j = 0; j < J_PRIME; j++) {
                    for (int k = 0; k < K_PRIME; k++) {
                        uint32_t *A_ptr = A_base + (16 * (j + i*J_PRIME));
                        uint32_t *W_ptr = W_base + (16 * (k + j*K_PRIME));
                        uint32_t *R_ptr = R_base + (16 * (k + i*K_PRIME));

                        bp_hprint_uint64(16 * (k + i*K_PRIME));
                        bp_cprint('\n');
