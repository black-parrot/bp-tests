...
//#define TEST_CASE_3
//#define TEST_CASE_4
//#define TRANSFORMER_PROJ_1
//#define TRANSFORMER_FC_1
//#define TRANSFORMER_FC_2
    ...
    #define I_SUPER_PRIME 1
    #define J_SUPER_PRIME 1
    #define K_SUPER_PRIME 1
    #define K_SKIP 1
    ...
    #define I_SUPER_PRIME 2
    #define J_SUPER_PRIME 2
    #define K_SUPER_PRIME 1
    ...
    #define I_SUPER_PRIME 1
    #define J_SUPER_PRIME (J / 4 / J_PRIME)
    #define K_SUPER_PRIME (K / 4 / K_PRIME)
    ...
    #define I_SUPER_PRIME (I / 4 / I_PRIME)
    #define J_SUPER_PRIME (J / 4 / J_PRIME)
    #define K_SUPER_PRIME (K / 4 / K_PRIME)
...
#elif defined(TRANSFORMER_PROJ_1)
    #define I 1
    #define J 768
    #define K 768
...
int main(int argc, char** argv) {
    for (int t = 0; t < 2; t++) {
        ...
        #if defined(USE_TC)
            for (uint64_t i2 = 0; i2 < I_SUPER_PRIME; i2++) {
                for (uint64_t j2 = 0; j2 < J_SUPER_PRIME; j2++) {
                    for (uint64_t k2 = 0; k2 < K_SUPER_PRIME; k2++) {

                        uint64_t super_offset_A = (j2*J_PRIME + i2*J_SUPER_PRIME) ;
                        uint64_t super_offset_W = (k2 + j2*K_SUPER_PRIME) ;
                        uint64_t super_offset_R = (k2 + i2*K_SUPER_PRIME) ;

                        for (uint64_t i = 0; i < I_PRIME; i++) {
                            for (uint64_t j = 0; j < J_PRIME; j++) {
                                for (uint64_t k = 0; k < K_PRIME; k+=K_SKIP) {
                                    uint64_t offset_A = ( i2*J_SUPER_PRIME*J_PRIME*I_PRIME + i*J_SUPER_PRIME*J_PRIME + j2*J_PRIME + j ) ;
                                    uint64_t offset_R = ( i2*K_SUPER_PRIME*K_PRIME*I_PRIME + i*K_SUPER_PRIME*K_PRIME + k2*K_PRIME + k ) ;
                                    uint64_t offset_W = ( j2*K_SUPER_PRIME*K_PRIME*J_PRIME + j*K_SUPER_PRIME*K_PRIME + k2*K_PRIME + k ) ;

                                    //  uint64_t offset_A = (i*J_PRIME + j) ;
                                    //  uint64_t offset_W = (j*K_PRIME + k) ;
                                    //  uint64_t offset_R = (i*K_PRIME + k) ;
                                    ...
                                    if (!buf) {
                                        ...
                                        if (K_SKIP == 1) {
                                            ...
                                            matmul2_b0(W_ptr, A_ptr);
                                        } else if (K_SKIP == 4)  {
                                            matmul4_b0(W_ptr, A_ptr);
                                        } else if (K_SKIP == 8)  {
                                            matmul8_b0(W_ptr, A_ptr);
                                        }
                                    } else {
                                        nop();
                                        nop();
                                        ...
                                        if (K_SKIP == 1) {
                                            ...
                                            matmul2_b1(W_ptr, A_ptr);
                                        } else if (K_SKIP == 4)  {
                                            matmul4_b1(W_ptr, A_ptr);
                                        } else if (K_SKIP == 8)  {
                                            matmul8_b1(W_ptr, A_ptr);
                                        }
                                    }
